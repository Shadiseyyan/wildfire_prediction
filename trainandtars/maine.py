...
Architecture:
    Input (B, T, H, W, C)
    → 3-D Patch Embedding
    → Earthformer Encoder  (local + global cuboid self-attention)
    → QML Bridge           (pooled VQC residual via PennyLane)
    → Earthformer Decoder  (cuboid cross-attention)
    → Prediction Heads     (spread, probability, ignition risk, burned area)

Dependencies:
    pip install torch pennylane einops numpy matplotlib seaborn
"""
